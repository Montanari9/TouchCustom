...
CONST TSL_Bank_T MyBanks[TSLPRM_TOTAL_BANKS] = {
  ...
  {&MyChannels_Src[7], &MyChannels_Dest[7], MyChannels_Data, BANK_2_NBCHANNELS, BANK_2_MSK_CHANNELS, BANK_2_MSK_GROUPS},
  ...
  if (idx_bank > TSLPRM_TOTAL_BANKS-1)
  {
    ...
    if (TSL_tim_CheckDelay_ms(100, &Gv_ECS_last_tick) == TSL_STATUS_OK)
    {
      if (TSL_ecs_Process(&MyObjGroup) == TSL_STATUS_OK)
      {
         for(i=0;i<10;i++)
         {
            if(MyTKeys[i].p_Data->StateId == TSL_STATEID_RELEASE)
            {
               TouchRefSum[i] -= TouchRefSum[i]>>2;
               TouchRefSum[i] += MyTKeys[i].p_ChD->Meas;
               TouchRef[i] = TouchRefSum[i]>>2;
            }
         }
      }
    }
